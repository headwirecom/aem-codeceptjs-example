...
const config = require("../project.conf.js");
...
module.exports = {
	...
	isInSources(absPath) {
		return absPath.startsWith(pathLib.normalize(sourcesDir + "/"));
	},
	...
	addVersion(absPath) {
		if (absPath && this.isInSources(absPath)) {
			let versionedPath = absPath.replace(sourcesDir, sourcesDir + "/" + aem.version);
